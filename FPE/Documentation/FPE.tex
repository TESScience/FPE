...
Figure \ref{Reset} shows the circuit that resets the FPGA to allow it to accept a program upload. The FPGA is normally in this state after power is turned on. Once programmed, the uplink is control data, not programming data. By driving the uplink clock high for $>$50ms, the controlling computer may restore the FPGA to the unprogrammed state. Normally, the controlling computer should either hold the uplink clock low, or cycle it to clock uplink data.
